# Thoughtful Systems Agent Plugin

This is the publishable ChatGPT, Codex, and Claude plugin package for Thoughtful Systems. It
...
- the production Thoughtful Systems Streamable HTTP MCP connection; and
- an Agent Skill for safely discovering, using, building, publishing, and
  operating governed Thoughtful Systems tools.
...
> Connect to Thoughtful Systems and show me the tools I can use.

The expected stable MCP tools are `toggle_build_mode`, `system_catalog`,
`system_use`, `system_tasks`, and `system_inbox`. Published app tools may also appear
directly in clients that refresh dynamic tool lists. `system_catalog` advertises
...
See [`skills/system/references/setup.md`](skills/system/references/setup.md) for
connection verification and troubleshooting.

## Skill evaluations

The repository-level
[`evals/system-skill`](https://github.com/thoughtful-systems/plugins/tree/main/evals/system-skill)
suite compares the same model and deterministic, production-shaped Thoughtful Systems tools
...
[expanded Luna evaluation report](https://github.com/thoughtful-systems/plugins/blob/main/evals/system-skill/results/2026-08-23-luna-complexity.md)
...
`system_catalog` before use. Request `system_catalog({ action: "authoring" })` for
...
`GET https://thoughtful.systems/status`; authentication remains client-managed
