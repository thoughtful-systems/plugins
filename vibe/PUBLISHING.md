# Vibe Marketplace Publishing

Vibe ships one canonical Agent Skill and one OAuth-protected Streamable HTTP
...
| Claude Connectors Directory | `https://vibe.sam.engineer/mcp` | Remote MCP server |
...
Also run the Vibe MCP tests that assert every advertised tool has a title and
...
- [ ] Decide whether the permanent marketplace MCP origin is
      `vibe.sam.engineer` or `thoughtful.systems` before review. Do not change a
...
- [ ] Capture listing icon, copy, categories, support contact, and any required
      screenshots. Screenshots become mandatory if Vibe adds MCP App UI.
