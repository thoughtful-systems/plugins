# Set up and verify Vibe

## Connection

The plugin declares one Streamable HTTP MCP server named `vibe`:
...
2. Ensure the client enables both the `vibe` skill and the `vibe` MCP server.
   A client may conform while supporting only one portable component type.
3. Start the connection. When prompted, complete Vibe's browser sign-in and
...
request reached Vibe as a verified user. A `401` or authorization challenge is
not success.
...
- Build/use mode is scoped to the current bearer credential and expires with
  it. It changes `vibe_catalog` and `vibe_use`, not the caller's permissions.
...
`https://vibe.sam.engineer/mcp` through its native MCP UI or use a client that
supports both components.

### Tools load but catalog access fails

Confirm the user completed consent for the intended Vibe organization. If the
...
Select build mode, then make a fresh `vibe_catalog` request. The mode switch
