...
`vibe_catalog` supports `list`, `search`, `describe`, and `types` in use mode.
...
`vibe_use` reports a stale catalog, changed version, revoked grant, disabled
tool, or schema mismatch:

1. Stop using the old method declaration.
2. Refresh `vibe_catalog` search/types.
...
`vibe_tasks` supports `assign`, `claim`, `get`, `edit`, `comment`, `accept`,
`decline`, `start`, `block`, `resume`, `complete`, and `cancel`. A team task has
one atomic claimant.

`vibe_inbox` supports `summary`, `list`, `get`, `send`, `mark_read`, and
